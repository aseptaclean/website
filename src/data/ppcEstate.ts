...
//
// ─────────────────────────────────────────────────────────────────────────────────────────────
// C. FREE WALKTHROUGH — NARROW OWNER-APPROVED EXCEPTION
// ─────────────────────────────────────────────────────────────────────────────────────────────
//
// AGENTS.md §7 bans "free assessment" and "free consultation" sitewide and states that the
// 2026-09-06 free-WALKTHROUGH exception originally reached only
// /hoarding-cleanup-san-jose/assessment/. The owner explicitly approved the same narrow wording
// for this route on 2026-09-09; AGENTS.md §4 and §7 now record both route-scoped exceptions.
//
...
// The required owner sign-off is complete. This does not change another route's price or label.
...
export const ppcEstate = {
  ...
  // ── TRUST BAR ──────────────────────────────────────────────────────────────────────────────
  // Below the hero and form. All three items were explicitly approved by the owner for this
  // route on 2026-09-09. This approval is route-scoped and changes no shared credential strip.
  //
  //   1. TSW #933 — APPROVED HERE. The registration is verified active (AGENTS.md §3). The owner
  //      widened its display scope to this trust item only; it does not add a trauma service to
  //      the estate campaign or authorize the credential on another campaign route.
  //
  //   2. INSURED — APPROVED HERE, using exactly the requested one-word claim. This route-scoped
  //      instruction does not set `PUBLIC_INSURANCE_STATUS`, publish the longer certificate or
  //      endorsement wording, or open insurance claims on any other route.
  //
...
// TSW #933 DISPLAY SCOPE ON THIS ROUTE. Explicitly approved by the owner on 2026-09-09 for this
// trust item only. The service and credential rules on every other route remain unchanged.
export const publishTswOnEstateCampaign = true;

// The exact one-word insurance item was approved for this route in the same instruction. Keep
// this route-level constant separate from `site.business.insuranceStatus`: setting the shared
// value would publish broader wording on unrelated pages without evidence or authorization.
export const publishInsuredOnEstateCampaign = true;

// The three owner-approved trust items, in the requested order and proportions.
export const ppcEstateTrustItems = [
  ...(publishTswOnEstateCampaign ? [ppcEstate.trustBar.tsw] : []),
  ...(publishInsuredOnEstateCampaign ? [ppcEstate.trustBar.insured] : []),
