...
- ~~§4 rhythm ratios — adjacent sections differ by ≥1.5x or are exactly 1.0x~~ **Amended by
  owner ruling 2026-08-20 — see §4.1 below.** Uniform section rhythm is permitted, and is
  compliant, wherever the approved mockups specify it. The 1.5× rule still governs rhythm
  choices the mockups do not settle.
...
## 4.1 Section rhythm — authority and scale

**Owner ruling 2026-08-20.** Two questions were open after the sitewide spacing audit. Both
are settled here.

### The mockups are the section-rhythm authority

`docs/mockups/*.html` are the pixel authority for section vertical rhythm. The operative
values, measured from those files rather than read off them:

| Wrapper | ≥761px | ≤760px |
| --- | --- | --- |
| `.sec` | 68px | 60px |
| `.sec--tight` | 48px | **40px** — see the correction note below |
| `.hero > .shell` | 86px (+36px curve allowance on `.hero` itself) | same |
| `.ph > .shell` | 66px / 62px | 44px |
| `.pgh > .shell` | 42px / 44px | same |
| `.fit > .shell` | 34px / 36px | same |
| `.cred` | 24px | 18px |
| `.fieldwrap` | 0 / 68px | same |
| `.band`, `.photoband` | 0 — height comes from `min-height: 96px` and the image | `.band` 22px |

### The 8px scale is a preference for new values, not a constraint on ratified ones

The spacing tokens run 8/16/24/32/48/64/96. Several ratified mockup values are not multiples
of 8 — 68, 60, 86, 34, 36, 24, 18, 22, 66, 62, 42, 44. **These are correct as-is and must not
be rounded.**

Rationale, recorded because this will be re-litigated otherwise: the ×8 grid is a device for
making *undecided* spacing consistent. Its value is that it removes arbitrary choices. Once a
value has been decided and ratified in an approved mockup, the grid has no remaining work to
do on it — rounding 68 to 72 would trade a specified value for an unspecified one and change
the rendered design to satisfy a rule whose only purpose was to prevent unspecified values.
The audit that raised this measured the whole site against the mockups and found the mockup
values internally coherent; the ×8 conflict was the instruction set disagreeing with itself,
not the design being wrong.

**So:** any *new* spacing value picks from the 8px scale. Any value that traces to an approved
mockup stands at its mockup figure. If a mockup value is itself wrong, that is a mockup defect
— correct the mockup and the build together (see below), do not silently round in the build.

### Uniform rhythm is compliant where the mockups specify it

The retained `11` §4 rule — adjacent sections differ by ≥1.5× or are exactly 1.0× — and the §9
failure-audit line "do any two adjacent sections share container width *and* vertical padding?"
both fail against the approved mockups themselves: the homepage mockup runs two consecutive
`.sec--tight` sections at identical padding in the identical 1200px shell.

**Uniform section rhythm is permitted, and is compliant, wherever the approved mockups specify
it.** `/about/`'s four consecutive 68px sections in the same 1200px shell are **compliant under
this amendment** and are not to be reported as a failure. Rhythm variation in this direction is
carried by the zero-padding bands (`.band`, `.photoband`, `.cred`, `.fit`) breaking up the 68px
runs, plus surface and colour change — not by modulating `.sec` itself.

The 1.5× rule continues to govern rhythm decisions the mockups do not settle.

### One mockup defect corrected, 2026-08-20

`.sec--tight` rendered **60px at 390px against 48px at 1440px** in all twelve mockups — looser
on the small screen than the large one, for the one variant that exists to be tighter. Cause:
`@media(max-width:760px) .sec{padding:60px 0}` is declared after `.sec--tight{padding:48px 0}`
at equal specificity and captures it. That is a media-query ordering bug, not a decision.

Owner ruling: **do not reproduce it.** The build is 48px desktop / 40px mobile, and all twelve
mockup files were corrected in the same change so the bug is not re-taught to whoever reads
them as authority next. Logged as a mockup defect corrected, **not** as a build divergence.
...
> Insured · Organic Pathogen Endorsed · Owner-Operated · ~~Santa Clara County~~ South Bay & Peninsula

**Fourth cell corrected 2026-08-20 — owner ruling B2.** "Santa Clara County" is struck sitewide:
the 10-city footprint includes Atherton, which is in San Mateo County, so the county label was
never true of the service area it described. The bar's *structure* — four verifiable cells, no
stat bar — is unchanged and still governs; only the fourth cell's wording moves, to the single
service-area string `src/data/site.ts` now carries (`site.location.regionLabel`). The component
already read that value, so nothing in `CredentialBar.astro` was retyped to match this.
...
- [ ] ~~Do any two adjacent sections share container width *and* vertical padding? → fail~~
      **Amended by owner ruling 2026-08-20 (§4.1).** Replaced by: do any two adjacent sections
      share container width *and* vertical padding **at a value the approved mockups do not
      specify**? → fail. Uniform rhythm the mockups do specify is compliant — `/about/`'s four
      consecutive 68px sections are the worked example.
