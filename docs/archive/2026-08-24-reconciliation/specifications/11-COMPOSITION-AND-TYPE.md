...
**Amended by owner ruling 2026-08-20 — see `docs/18-VISUAL-DIRECTION.md` §4.1, which is the
operative statement.** `docs/mockups/*.html` are the section-rhythm authority, and **uniform
rhythm is compliant wherever those mockups specify it.** The mockups run `.sec` at a single
68px/60px value with variation carried by the zero-padding bands (`.band`, `.photoband`,
`.cred`, `.fit`) and by surface and colour change, not by modulating `.sec`. The 1.5× ratio
still governs rhythm choices the mockups do not settle.

The token table above is also **historical**: the shipped build reads its section rhythm from
`.ac-sec` / `.ac-sec--tight` in `src/styles/global.css`, not from `--ac-rhythm-*`. Those five
tokens have no consumer on any indexable route.
...
- [ ] ~~Do any two adjacent sections share container width *and* vertical padding? → fail~~
      **Amended by owner ruling 2026-08-20** — see `docs/18-VISUAL-DIRECTION.md` §4.1.
      Replaced by: at a value the approved mockups do **not** specify? → fail.
