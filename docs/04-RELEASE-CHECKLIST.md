...
- [x] Primary CTA is consistently `Get My 24-Hour Handoff Plan`. Full label used in Hero/FinalCTA (`primaryCta`); header/sticky nav intentionally uses the shorter `compactCta` ("Get My Handoff Plan") for space — same offer/destination, not a competing CTA. Documented as a deliberate variant, not a defect.
