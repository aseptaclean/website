# Aseptaclean — Approved Detailed Deep Cleaning Page

Approval: September 19, 2026. The owner locked the revised Detailed Deep Cleaning interactive mockup with “lock it, update zip.” This reference supersedes the earlier copy and layout directions.

## Authority and files

- `aseptaclean-detailed-deep-approved.html`: standalone visual reference. Open in a browser. Its outer iframe is an export wrapper, not website implementation markup.
- `detailed-deep-approved-fragment.html`: exact approved source, including embedded illustrative imagery, scoped CSS, markup, and demo interactions.
- `aseptaclean-detailed-deep-approved.css`: exact CSS extracted from the approved source, in original order. Later declarations intentionally resolve earlier layout and contrast rules.
...
1. Read repository instructions and inspect the existing detailed-cleaning route, shared layout, navigation, form component, and styles. Preserve the canonical route rather than inventing a new URL.
...
4. Use `#ac-detailed` as the styling boundary. Scope header, main, and footer consistently. Adapt working shared components without breaking their behavior or restyling unrelated pages.
...
2. Full-width photographic hero with left navy overlay. Heading: line 1 “Detailed Deep Cleaning.”; line 2 “Care in Every Detail.” Preserve separate block spans. The approved hero text container is 960px maximum; allow natural wrapping on smaller screens. Supporting copy and two CTAs match the source.
3. Attached three-part trust strip: Respectful Service; Written Plan & Price; Documented Work.
4. Six cleanup situations in a three-column, two-row desktop grid: Kitchens & Cabinets; Bathrooms; Floors & Edges; Doors & Baseboards; Interior Windows & Sills; Move-In & Move-Out.
5. Warm-background support section: “The details make the difference.” Full-width heading above text left and landscape illustrative photograph right. Three supporting points run across BELOW both columns.
...
9. Warm-background scope section: “Know what your cleaning includes.” Full-width heading above both columns, landscape photograph right. The two scope notes sit BELOW the split in a full-width two-column row.
...
- Display availability as 7 AM–7 PM. No numeric prices or assessment fee, fabricated reviews, guaranteed insurance coverage, response-time promises, or blanket safety guarantees.
...
- The hero uses the kitchen floor-cleaning tile from the embedded image sheet (100% 50% background position). Preserve this approved illustrative composition until suitable real photography is selected.
...
Source checks covered matching approved markup, CSS and copy extraction, preserved heading blocks, anchor targets, unique IDs, script syntax, 34-city list, absence of numeric prices, local demo behavior, unchanged embedded imagery, checksums, and archive integrity. Browser rendering and live interaction checks remain outstanding because local preview navigation was blocked in this environment. This package is an approved design reference, not a deployed or production-tested page.
...
Detailed deep cleaning focuses on accessible rooms and surfaces: kitchens, bathrooms, floors, edges, doors, trim, baseboards, and move-in/move-out needs. Interior windows, inside cabinets, and inside appliances must be explicitly included in the scope. Large cleanouts, heavy waste, and biohazard conditions require a separate assessment and scope. Repairs, pest control, and replacing damaged materials are excluded. Cleaning does not promise removal of every stain, restoration of worn finishes, or correction of damage. No starting prices appear.

## Locked headline

Line 1: Detailed Deep Cleaning.
Line 2: Care in Every Detail.

Keep these as two separate block spans, allowing natural wrapping within each block on small screens. This version supersedes the earlier Detailed Cleaning name for this page.

## Current copy boundaries

Records are limited to the job scope. Cleaning behind heavy furniture or appliances is not assumed to be included. Do not add carpet extraction, upholstery cleaning, exterior windows, appliance moving, or recurring housekeeping without owner confirmation. Preserve the limits on stain removal and restoration of worn finishes.
