...
Status: updated at the owner’s request on 2026-09-19 with the latest reviewed copy and refined pricing mockup.
Scope: public Rodent & Animal Waste service page, not an advertising landing page.
Authority: the latest rodent service-page mockup shown in this conversation, including the owner’s corrected starting prices and subsequent pricing-layout revision. This supersedes the older reference in this ZIP.

## Source of truth

- `rodent-service-preview.html`: standalone interactive visual reference. Open this first.
- `rodent-service-fragment.html`: exact approved markup, styles, demo interactions, and embedded image assets.
- `rodent-service.css`: every style block extracted in its original cascade order. The last rules deliberately override earlier rules. Do not drop these overrides.
...
5. Attics and crawl spaces: full-width heading, text/image pair, then three supporting points spanning both columns.
6. Your belongings matter: full-width heading above the text/image row, then three steps underneath.
7. Navy assessment CTA band.
8. Our process: four steps.
9. Before work begins: full-width heading, text/image pair, two cleanup prices, pricing explanation, assessment band with CTA, then two scope columns.
10. Common questions: centered heading, single accordion column, contact row below.
11. Service area: 15 Santa Clara County cities and 19 San Mateo County cities; South San Francisco excluded.
...
Place the eyebrow and heading across the full container above the text/image grid. Keep the explanatory copy on the left and the documentation image on the right at a 3:2 landscape ratio, with no minimum height. Preserve the proportional crop that hides the image’s bottom strip. Never stretch the image to match text height.

Below the pair, use this order:
1. Two equal columns: Small cleanup jobs / Starting at / $500, and Larger cleanup jobs / Starting at / $1,500. The amount has stronger emphasis than “Starting at”.
2. The final-price explanation beneath both columns.
3. A separate slate assessment band: “On-site assessment · $145”, the credit explanation, and the section’s Request an Assessment CTA. Do not restore the earlier CTA above pricing or the three-equal-price-column layout.
4. Cleanup & Removal and Separate Services in two columns with thin borders, not thick blue rules.

At 620px or less, stack the prices, assessment content/button, and scope columns. Preserve the image proportions. The exact fragment and CSS control spacing and typography.

### Full-width headings and attic points

The Where we help, Attics & crawl spaces, Your belongings matter, and Before work begins headings have full available width and no forced line breaks. Give them space rather than shrinking type or forcing overflow. Preserve the hero’s two-line break and allow natural wrapping on mobile.

The attic’s three supporting points sit beneath BOTH text and photo, across three columns on desktop and one column on mobile. Keep the existing full-width belongings steps as well.
...
- Do not offer insulation replacement, pest control, entry-point sealing, or repairs.
- Preserve the distinction between affected belongings that can be cleaned and materials proposed for removal.
- Owner-approved prices: small cleanup jobs start at $500; larger cleanup jobs start at $1,500. These categories refer to job size, not attic/crawl-space location. No price ranges or $4,000 ceiling.
- On-site assessment: $145, credited toward the cleanup if the customer proceeds. Preserve this in the pricing band, cost FAQ, and form introduction. Do not invent expiry dates or additional credit conditions.
- Do not add health guarantees, certifications, reviews, technical methods, testing/clearance claims, insurance payment, or specific unconfirmed animal-waste capabilities.
- No company-owned hauling claim. Disposal logistics remain job-specific.
...
Source/package checks completed: one H1; unique IDs and valid in-page anchors; six native FAQ disclosures; 34 cities; labeled controls; preserved local demo script; JavaScript syntax; correct starting prices and assessment fee; CSS synchronized with fragment; original embedded image bytes preserved; updated checksums and ZIP integrity.

Browser rendering and interaction testing remain unverified. Earlier browser attempts could not access the local preview and file-protocol navigation was rejected by security policy. Do not claim desktop/mobile rendering, contrast, overflow, keyboard interaction, or form-validation browser checks passed. Complete those checks in an accessible authorized test environment before production handoff.

No production website code was edited, pushed, or deployed. No real lead was sent. Updating this ZIP does not authorize deployment.
